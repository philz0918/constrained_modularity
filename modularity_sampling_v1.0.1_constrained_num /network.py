...
class Network(object):
    ...
    def get_modularity_matrix_numpy(self):
        if numpy.all(self.modularity_matrix_numpy) == None:
...
class UndirectedNetwork(Network):
    ...
    def create_graphml_file(self, filename, grouping = None):
        ...
        if c <= len(colors):
            ...
            palette = ["gray"+str(x) for x in range(0,100)]
...
def create_bbtree_network(number_of_levels, probability_function = simple_probability_function):
    ...
    for i in range(n-1, -1, -1):
        ...
        for j in range(0, 2**i):
            ...
            for u in range(base_index, base_index + subgroup_size):
                for v in range(base_index + subgroup_size, base_index + 2*subgroup_size):
...
def create_flat_hrg(p_in):
    ...
    for group in range(0, number_of_groups):        
        for i in range(group*group_size, (group+1)*group_size):
            for j in range(group*group_size, i):
    ...
    for i in range(0, number_of_groups):
        for j in range(0, i):
            for u in range(i*group_size, (i+1)*group_size):
                for v in range(j*group_size, (j+1)*group_size):
